...
def pseudo_label_generator_acdc(data, seed, beta=50.00, mode='bf', img_class='odoc'):
    ...
    if img_class=='faz' or img_class == 'polyp' or img_class == 'isic' or img_class == 'busi':
...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', transform=None, client="client1", sup_type="label",img_class='prostate'):
        ...
        elif self.img_class =='polyp' or self.img_class == 'isic' or self.img_class == 'busi':
...
def random_rot_flip(image, label, img_class):
    if img_class == 'odoc' or img_class == 'odoc_binary' or img_class == 'polyp' or img_class == 'isic' or img_class == 'busi':
...
def random_rotate(image, label,img_class='odoc'):
    ...
    if img_class=='polyp' or img_class == 'isic' or img_class == 'busi':
        angle = np.random.randint(-45, 45)
        image = ndimage.rotate(image, angle, axes=(1,2), order=0, reshape=False)
        label = ndimage.rotate(label, angle, axes=(0,1), order=0,reshape=False, mode="constant", cval=2)
        return image, label
