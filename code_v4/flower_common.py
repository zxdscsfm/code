    ...
    def fit(self, num_rounds, timeout):
        ...
        def get_client_state_dict(central_parameters, client_parameters):
            ...
            if self.args.strategy in ['FedBN', 'FedAP', 'FedAPLC']:
                ...
                local_keys = get_feduniv2_local_keys(self.args.model, self.args.in_chns, self.args.num_classes)
            else:
                local_keys = []
...
def get_feduniv2_local_keys(model_name, in_chns, num_classes):
    assert model_name in ['unet_univ2', 'unet_univ3']
    if model_name == 'unet_univ2' or model_name == 'unet_univ3':
        local_keys = ['decoder.out_conv.weight', 'decoder.out_conv.bias']
    else:
        local_keys = []
...
class MyModel(nn.Module):
    ...
    def __init__(self, args, model, trainloader, valloader):
        ...
        if self.args.strategy in ['FedBN', 'FedAP', 'FedAPLC']:
            ...
            self.local_keys = get_feduniv2_local_keys(self.args.model, self.args.in_chns, self.args.num_classes)
            ...
            # FedUniV2, FedUniV2.1
                ...
                if config['stage'] == 'fit':
                    if config['iter_global'] <= self.args.iters:
                        ...
                        prompt_idx = num_weights + self.args.min_num_clients * num_weights + 1
                        ...
                        if self.args.dual_init in ['random', 'adjacent', 'nearest']:
                            if self.args.dual_init == 'random':
                                ...
                                a = prompts_array.reshape(prompts_array.shape[0], 1, -1)
                                b = prompts_array[self.args.cid].reshape(1, -1, 1)
                                c = a @ b
                                c = c.flatten()
