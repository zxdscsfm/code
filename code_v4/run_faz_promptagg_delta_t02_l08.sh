#!/bin/bash
#SBATCH --job-name=FedLPPA_FAZ_PromptAggDelta_t02_l08
...
LOG_DIR="logs/run_promptagg_delta_t02_l08_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
--in_chns 1 \
--img_class faz \
--exp faz/FedLPPA_promptagg_v2_delta_t02_l08 \
--model unet_univ5 \
...
--prompt_affinity_mode delta \
--prompt_agg_min_spread 1e-3 \
--verbose_logging 1"

echo "Starting FedLPPA PromptAgg delta t=0.2 lambda=0.8 control experiment. Logs: $LOG_DIR"
...
wait
echo "PromptAgg delta t02_l08 control run finished."
