#!/bin/bash
# Official FedLPPA + WANN + RGFTD-v1 on FAZ.

#SBATCH --job-name=FedLPPA_FAZ_RGFTD
...
RUN_PREFIX="${RUN_PREFIX:-official_rgftd_v1_faz_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8520}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting official FedLPPA + WANN + RGFTD-v1 FAZ run"
...
wait
echo "Official FedLPPA + WANN + RGFTD-v1 FAZ run finished. EXP_NAME=${EXP_NAME}"
