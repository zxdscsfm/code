#!/bin/bash
# stage2-v4: supervision-prior-guided prompt aggregation on top of v1
# Keep the original FedLPPA/v1 local training path and only revise server-side
# prompt aggregation with supervision prior scores. Auxiliary decoder
# aggregation stays on the original similarity-only path.
...
RUN_TAG="v2_prioragg_promptonly_soft_$(date +%Y%m%d_%H%M%S)"
...
--quality_prior_decay_start_iter 10000 \
--quality_prior_decay_end_iter 20000 \
--sup_type_list ${SUP_TYPES} \
--verbose_logging 1"

echo "Starting stage2-v4 prior-guided prompt-only FedLPPA run. EXP=${EXP_NAME} Logs=${LOG_DIR}"
...
wait
echo "Stage2-v4 prior-guided prompt-only FedLPPA run finished. EXP=${EXP_NAME}"
