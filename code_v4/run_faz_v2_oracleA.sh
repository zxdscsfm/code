#!/bin/bash
# Oracle A: use server-side oracle validation Dice to reweight global aggregation.
# This is a diagnostic experiment. It only changes Eq.2-style global aggregation
# and keeps the existing FedLPPA/FedUniV2.1 prompt personalization path intact.

#SBATCH --job-name=FedLPPA_FAZ_OracleA
...
RUN_TAG="v2_oracleA_promptonly_$(date +%Y%m%d_%H%M%S)"
...
echo "Starting Oracle A prompt-only FedLPPA run. EXP=${EXP_NAME} Logs=${LOG_DIR}"
...
wait
echo "Oracle A prompt-only FedLPPA run finished. EXP=${EXP_NAME}"
