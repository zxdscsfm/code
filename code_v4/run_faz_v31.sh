#!/bin/bash
#SBATCH --job-name=FedLPPA_FAZ_V31
...
RUN_TAG="v3_1_ccr_minimal_$(date +%Y%m%d_%H%M%S)"
...
--oracle_pseudo_label_enabled 0 \
--comp_prompt_enabled 1 \
--comp_prompt_slots 4 \
--comp_prompt_lambda 0.35 \
--comp_prompt_exclude_self 1 \
--sup_type_list ${SUP_TYPES} \
--verbose_logging 1"

echo "Starting v3.1 CCR minimal FedLPPA run. EXP=${EXP_NAME} Logs=${LOG_DIR}"
...
wait
echo "v3.1 CCR minimal FedLPPA run finished. EXP=${EXP_NAME}"