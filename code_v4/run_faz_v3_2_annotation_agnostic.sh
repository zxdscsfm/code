...
# FAZ is kept as a sanity-check dataset. The main value of this script is to
# verify the new setting runs end-to-end without using annotation-type metadata.

#SBATCH --job-name=FedLPPA_FAZ_V32Agnostic
...
RUN_TAG="v3_2_faz_annotation_agnostic_$(date +%Y%m%d_%H%M%S)"
...
echo "Starting v3.2 annotation-agnostic FedLPPA sanity run on FAZ. EXP=${EXP_NAME} Logs=${LOG_DIR}"
...
wait
echo "v3.2 annotation-agnostic FAZ run finished. EXP=${EXP_NAME}"
