#!/bin/bash
# Micro FAZ benchmark for fast smoke tests.

#SBATCH --job-name=FedLPPA_micro_FAZ
...
RUN_TAG="micro_faz_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="faz/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8101}"
ITERS="${ITERS:-5}"
...
echo "Starting micro FAZ benchmark"
...
wait
echo "Micro FAZ benchmark finished. EXP_NAME=${EXP_NAME}"
