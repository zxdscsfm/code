#!/bin/bash
# Micro ODOC benchmark for main development iterations.

#SBATCH --job-name=FedLPPA_micro_ODOC
...
RUN_TAG="micro_odoc_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8102}"
ITERS="${ITERS:-5}"
...
echo "Starting micro ODOC benchmark"
...
wait
echo "Micro ODOC benchmark finished. EXP_NAME=${EXP_NAME}"
