#!/bin/bash
# Official-code proportional micro ODOC baseline.

#SBATCH --job-name=FedLPPA_official_prop_micro_ODOC
...
cd /data/jianbingshen/yanghongji/FedLPPA_github_official_micro/code_v4

RUN_TAG="official_prop_micro_odoc_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8113}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-5}"
...
echo "Starting official-code proportional micro ODOC baseline"
...
wait
echo "Official-code proportional micro ODOC baseline finished. EXP_NAME=${EXP_NAME}"
