#!/bin/bash
# Minimal prototype-margin pseudo-label filtering on top of official micro ODOC FedLPPA.

#SBATCH --job-name=FedLPPA_protofilter_micro_ODOC
...
RUN_TAG="protofilter_v1_micro_odoc_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8115}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-5}"
...
echo "Starting prototype-filter v1 micro ODOC run"
...
wait
echo "Prototype-filter v1 micro ODOC run finished. EXP_NAME=${EXP_NAME}"
