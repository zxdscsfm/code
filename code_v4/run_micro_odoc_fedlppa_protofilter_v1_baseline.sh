#!/bin/bash
# Matched official_full micro ODOC baseline for prototype-filter ablation.

#SBATCH --job-name=FedLPPA_protobase_micro_ODOC
...
RUN_TAG="protofilter_v1_baseline_micro_odoc_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8116}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-5}"
...
echo "Starting matched official_full micro ODOC baseline"
...
wait
echo "Matched official_full micro ODOC baseline finished. EXP_NAME=${EXP_NAME}"
