#!/bin/bash
# Official-code full-data ODOC with prototype-margin pseudo-label filtering.

#SBATCH --job-name=FedLPPA_protofilter_full_ODOC
...
RUN_TAG="protofilter_v1_full_odoc_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8123}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-5}"
EVAL_ITERS="${EVAL_ITERS:-50}"
TSNE_ITERS="${TSNE_ITERS:-200}"
MAX_ITERATIONS="${MAX_ITERATIONS:-30000}"
PROTO_WARMUP="${PROTO_WARMUP:-50}"
PROTO_TOPK="${PROTO_TOPK:-0.25}"
...
--label_prompt 1 \
--prototype_filter_enabled 1 \
--prototype_filter_warmup_rounds ${PROTO_WARMUP} \
--prototype_filter_topk_ratio ${PROTO_TOPK}"

echo "Starting official-code full-data ODOC prototype-filter v1"
echo "EXP_NAME=${EXP_NAME}"
echo "LOG_DIR=${LOG_DIR}"
echo "SEED=${SEED}"
echo "SERVER_ADDRESS=${SERVER_ADDRESS}"
echo "MAX_ITERATIONS=${MAX_ITERATIONS} ITERS=${ITERS} EVAL_ITERS=${EVAL_ITERS} TSNE_ITERS=${TSNE_ITERS}"
echo "Prototype filter: warmup=${PROTO_WARMUP} topk=${PROTO_TOPK}"
...
wait
echo "Official-code full-data ODOC prototype-filter v1 finished. EXP_NAME=${EXP_NAME}"
