#!/bin/bash
# Official FedLPPA + WANN + RGFTD-v3.5 stable-teacher routing on ODOC.

#SBATCH --job-name=FedLPPA_ODOC_RGFTD_STABLE
...
RUN_PREFIX="${RUN_PREFIX:-official_rgftd_stable_odoc_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8531}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
ROOT_PATH="${ROOT_PATH:-/data/jianbingshen/yanghongji/FedLPPA_github_official_full/data/ODOC_h5}"
...
echo "Starting official FedLPPA + WANN + RGFTD-v3.5 stable ODOC run"
...
python -u flower_pCE_2D_v4_FedLPPA.py ${BASE_ARGS} --role client --cid 4 --client client5 --sup_type block --gpu 5 --num_workers 0 > "${LOG_DIR}/client4.log" 2>&1 &

wait
echo "Official FedLPPA + WANN + RGFTD-v3.5 stable ODOC run finished. EXP_NAME=${EXP_NAME}"
