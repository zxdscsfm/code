#!/bin/bash
# Official FedLPPA + WANN + RGFTD-v2 on ODOC.

#SBATCH --job-name=FedLPPA_ODOC_RGFTD
...
RUN_PREFIX="${RUN_PREFIX:-official_rgftd_v2_odoc_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8521}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting official FedLPPA + WANN + RGFTD-v2 ODOC run"
...
wait
echo "Official FedLPPA + WANN + RGFTD-v2 ODOC run finished. EXP_NAME=${EXP_NAME}"
