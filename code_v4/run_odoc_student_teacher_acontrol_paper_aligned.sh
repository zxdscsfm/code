#!/bin/bash
# Full-data ODOC A-control run for the student-teacher baseline,
# translated to the current code semantics from the paper-style
# protocol: 500 federated rounds x 10 local iterations = 5000 total steps.

#SBATCH --job-name=FedLPPA_STv1_Actrl_Paper
...
RUN_PREFIX="${RUN_PREFIX:-stAcontrol_paper_r500_l10_odoc}"
RUN_TAG="${RUN_PREFIX}_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8152}"
SEED="${SEED:-2022}"

# Paper-aligned training budget under the current code semantics:
# 500 federated rounds x 10 local iterations -> 5000 total optimizer steps.
ITERS="${ITERS:-10}"
EVAL_ITERS="${EVAL_ITERS:-10}"
TSNE_ITERS="${TSNE_ITERS:-200}"
MAX_ITERATIONS="${MAX_ITERATIONS:-5000}"

KD_LAMBDA="${KD_LAMBDA:-0.5}"
...
# A-control: keep the student-teacher baseline path, turn off new mechanisms.
KD_VETO="${KD_VETO:-0}"
...
echo "Starting paper-aligned A-control ODOC run"
...
echo "KD_VETO=${KD_VETO} REFRESH_VETO=${REFRESH_VETO} ASYM_PHOTOMETRIC=${ASYM_PHOTOMETRIC} EMA_ENABLED=${EMA_ENABLED}"
...
wait
echo "Paper-aligned A-control ODOC run finished. EXP_NAME=${EXP_NAME}"
