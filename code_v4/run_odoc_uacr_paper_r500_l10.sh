#!/bin/bash
# ODOC UACR:
# v1B backbone + dg2 geometry routing + uncertainty-aware collaborative refinement.

#SBATCH --job-name=FedLPPA_ODOC_UACR
...
METHOD_TAG="${METHOD_TAG:-uacr_odoc_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8397}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--uacr_enabled 1 \
--uacr_geometry_gate_power 2.0 \
--uacr_warmup_rounds 800 \
--uacr_ramp_rounds 800 \
--uacr_lambda_scale 1.0 \
--uacr_certainty_floor 0.05 \
--uacr_require_agreement 0 \
--uacr_consensus_conf_thr 0.50 \
--uacr_consensus_margin_thr 0.10 \
--uacr_min_uncertain_pixels 16 \
--uacr_min_candidate_pixels 8 \
--uacr_server_min_count 32 \
--uacr_server_count_tau 256.0 \
--uacr_server_min_supporters 2 \
--uacr_server_client_min_count 8 \
--uacr_server_client_cap_count 32 \
--uacr_server_template_momentum 0.5 \
--uacr_server_stale_decay 0.9 \
${EXTRA_ARGS}"

echo "Starting ODOC UACR run"
...
echo "ODOC UACR run finished. EXP_NAME=${EXP_NAME}"
