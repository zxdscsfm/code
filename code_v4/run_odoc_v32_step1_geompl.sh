#!/bin/bash
# Full-data ODOC run for v3.2-step1:
# geometry binning + geometry-guided pseudo-label weighting only.

#SBATCH --job-name=FedLPPA_ODOC_step1
...
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8112}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-50}"
EVAL_ITERS="${EVAL_ITERS:-50}"
TSNE_ITERS="${TSNE_ITERS:-500}"
...
echo "Starting full-data ODOC v3.2-step1 run"
...
wait
echo "Full-data ODOC v3.2-step1 run finished. EXP_NAME=${EXP_NAME}"
