#!/bin/bash
# ODOC W1:
# geometry-guided pseudo-label weighting + adaptive tau-gated hard/soft pseudo-label learning
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W1
...
METHOD_TAG="${METHOD_TAG:-v33_w1_adaptau_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8172}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting ODOC W1 adaptive tau run"
...
wait
echo "ODOC W1 adaptive tau run finished. EXP_NAME=${EXP_NAME}"
