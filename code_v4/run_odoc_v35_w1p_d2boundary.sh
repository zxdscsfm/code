#!/bin/bash
# ODOC W3 / D2:
# W1' local-global calibrated adaptive pseudo-labeling
# + OC-only boundary-focused supervision on reliable unlabeled geometry regions

#SBATCH --job-name=FedLPPA_ODOC_D2
...
METHOD_TAG="${METHOD_TAG:-v35_w1p_d2boundary_paper_r500_l10}"
...
BOUNDARY_LAMBDA="${BOUNDARY_LAMBDA:-0.3}"
BOUNDARY_KERNEL_SIZE="${BOUNDARY_KERNEL_SIZE:-3}"
BOUNDARY_WARMUP_ITERS="${BOUNDARY_WARMUP_ITERS:-800}"
...
--adaptive_pl_boundary_lambda ${BOUNDARY_LAMBDA} \
--adaptive_pl_boundary_kernel_size ${BOUNDARY_KERNEL_SIZE} \
--adaptive_pl_boundary_warmup_iters ${BOUNDARY_WARMUP_ITERS}"

echo "Starting ODOC W3 D2 boundary run"
...
echo "BOUNDARY_LAMBDA=${BOUNDARY_LAMBDA} BOUNDARY_KERNEL_SIZE=${BOUNDARY_KERNEL_SIZE} BOUNDARY_WARMUP_ITERS=${BOUNDARY_WARMUP_ITERS}"
...
wait
echo "ODOC W3 D2 boundary run finished. EXP_NAME=${EXP_NAME}"
