#!/bin/bash
# ODOC W2_mid:
# W1' + local seed support + local feature-affinity propagation
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W2
...
METHOD_TAG="${METHOD_TAG:-v37_w2_seedprop_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8173}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--seed_support_enabled 1 \
--seed_support_warmup_iters 800 \
--seed_support_reliable_score_min 0.7 \
--seed_support_kernel_size 5 \
--seed_support_affinity_temp 8.0 \
--seed_support_blend_alpha 0.5 \
--seed_support_hard_tau 0.35 \
--seed_support_soft_tau 0.20"

echo "Starting ODOC W2_mid seed-support adaptive PL run"
...
wait
echo "ODOC W2_mid seed-support adaptive PL run finished. EXP_NAME=${EXP_NAME}"
