...
# ODOC is the more meaningful benchmark than FAZ for this setting because
# there is still real room above the original baseline.

#SBATCH --job-name=FedLPPA_ODOC_V32Agnostic
...
RUN_TAG="v3_2_odoc_annotation_agnostic_$(date +%Y%m%d_%H%M%S)"
EXP_NAME="odoc/FedLPPA_${RUN_TAG}"
LOG_DIR="logs/run_${RUN_TAG}"
mkdir -p "$LOG_DIR"

SUP_TYPES="scribble scribble_noisy scribble_noisy keypoint block"
...
echo "Starting v3.2 annotation-agnostic FedLPPA run on ODOC. EXP=${EXP_NAME} Logs=${LOG_DIR}"
...
python flower_pCE_2D_v4_FedLPPA.py $BASE_ARGS --role client --cid 0 --client client1 --sup_type scribble --gpu 1 > "$LOG_DIR/client0.log" 2>&1 &
python flower_pCE_2D_v4_FedLPPA.py $BASE_ARGS --role client --cid 1 --client client2 --sup_type scribble_noisy --gpu 2 > "$LOG_DIR/client1.log" 2>&1 &
python flower_pCE_2D_v4_FedLPPA.py $BASE_ARGS --role client --cid 2 --client client3 --sup_type scribble_noisy --gpu 3 > "$LOG_DIR/client2.log" 2>&1 &
python flower_pCE_2D_v4_FedLPPA.py $BASE_ARGS --role client --cid 3 --client client4 --sup_type keypoint --gpu 4 > "$LOG_DIR/client3.log" 2>&1 &
python flower_pCE_2D_v4_FedLPPA.py $BASE_ARGS --role client --cid 4 --client client5 --sup_type block --gpu 5 > "$LOG_DIR/client4.log" 2>&1 &

wait
echo "v3.2 annotation-agnostic ODOC run finished. EXP=${EXP_NAME}"
