#!/bin/bash
# ODOC W4.2:
# Keep W4.1 early/mid behavior, then switch to gated sequential routing after 1200.
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W42
...
METHOD_TAG="${METHOD_TAG:-v42_w4_releasegate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8175}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting ODOC W4.2 staged client-state-aware adaptive PL run"
...
wait
echo "ODOC W4.2 staged client-state-aware adaptive PL run finished. EXP_NAME=${EXP_NAME}"
