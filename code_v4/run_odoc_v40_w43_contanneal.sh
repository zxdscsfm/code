#!/bin/bash
# ODOC W4.3:
# Keep W4 early correction statistics, but replace W4.2 discrete routing with
# continuous late-stage dual-channel annealing on top of the W1' training trunk.
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W43
...
METHOD_TAG="${METHOD_TAG:-v43_w1p_contanneal_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8176}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--risk_calibration_anneal_end_iter 1800 \
--risk_calibration_hard_min_correction 0.05 \
--risk_calibration_hard_risk_scale 0.15 \
--risk_calibration_calibration_min_correction 0.10 \
--risk_calibration_calibration_risk_scale 0.40 \
...
echo "Starting ODOC W4.3 continuous-anneal adaptive PL run"
...
wait
echo "ODOC W4.3 continuous-anneal adaptive PL run finished. EXP_NAME=${EXP_NAME}"
