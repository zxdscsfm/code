#!/bin/bash
# ODOC W5:
# W1' backbone + early/mid soft-only risk-aware calibration (hard branch stays W1-like)
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W5
...
METHOD_TAG="${METHOD_TAG:-v50_w5_w1p_softcalib_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8177}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting ODOC W5 W1'-backbone soft-only risk calibration run"
...
wait
echo "ODOC W5 W1'-backbone soft-only risk calibration run finished. EXP_NAME=${EXP_NAME}"
