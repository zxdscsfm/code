#!/bin/bash
# ODOC W8:
# use W4.2-style early correction before 800, then smoothly bridge adaptive PL loss to pure W1'.
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W8
...
METHOD_TAG="${METHOD_TAG:-v54_w8_w42bootstrap_w1p_softbridge_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8181}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
W8_TRANSITION_START="${W8_TRANSITION_START:-800}"
W8_TRANSITION_END="${W8_TRANSITION_END:-1100}"
...
--risk_calibration_w8_enabled 1 \
--risk_calibration_w8_transition_start_iter ${W8_TRANSITION_START} \
--risk_calibration_w8_transition_end_iter ${W8_TRANSITION_END} \
--risk_calibration_w7_enabled 0 \
--risk_calibration_w5_soft_only_enabled 0 \
--risk_calibration_w6_late_tail_enabled 0"

echo "Starting ODOC W8 run (W4.2-style bootstrap before ${W8_TRANSITION_START}, soft bridge to pure W1' by ${W8_TRANSITION_END})"
...
echo "W8_TRANSITION_START=${W8_TRANSITION_START} W8_TRANSITION_END=${W8_TRANSITION_END}"
...
wait
echo "ODOC W8 run finished. EXP_NAME=${EXP_NAME}"
