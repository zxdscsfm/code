#!/bin/bash
# ODOC W9:
# 0~800 keep W4-style bootstrap, 800~1100 stay on a single W4 path but anneal controls down,
# 1100+ switch to pure W1' without mixing losses.
# paper-aligned schedule: global 5000 / local 10

#SBATCH --job-name=FedLPPA_ODOC_W9
...
METHOD_TAG="${METHOD_TAG:-v55_w9_w42anneal_w1p_singlepath_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8182}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
W9_TRANSITION_START="${W9_TRANSITION_START:-800}"
W9_TRANSITION_END="${W9_TRANSITION_END:-1100}"
...
--risk_calibration_tau_lambda 0.12 \
--risk_calibration_conf_lambda 0.08 \
--risk_calibration_prior_power 1.0 \
--risk_calibration_prior_clip_min 0.5 \
--risk_calibration_prior_clip_max 1.5 \
--risk_calibration_hard_dampen 0.5 \
--risk_calibration_soft_boost 0.5 \
--risk_calibration_global_soft_lambda 0.15 \
--risk_calibration_stateful_release_enabled 1 \
--risk_calibration_release_start_iter ${W9_TRANSITION_START} \
--risk_calibration_release_full_iter ${W9_TRANSITION_END} \
--risk_calibration_anneal_end_iter ${W9_TRANSITION_END} \
--risk_calibration_regime_ema_momentum 0.9 \
--risk_calibration_release_risk_threshold 0.55 \
--risk_calibration_release_agreement_threshold 0.94 \
--risk_calibration_release_prior_gap_threshold 0.020 \
--risk_calibration_release_uncertain_threshold 0.03 \
...
--risk_calibration_release_max_correction 0.08 \
--risk_calibration_preserve_min_correction 0.12 \
--risk_calibration_preserve_max_correction 0.25 \
--risk_calibration_correction_min_correction 0.70 \
--risk_calibration_regime_hysteresis 0.05 \
--risk_calibration_routing_mode gated \
--risk_calibration_gate_release_start_iter 1200 \
--risk_calibration_release_streak_required 3 \
--risk_calibration_release_streak_decay 1 \
--risk_calibration_release_hard_ratio_threshold 0.60 \
--risk_calibration_w9_enabled 1 \
--risk_calibration_w9_transition_start_iter ${W9_TRANSITION_START} \
--risk_calibration_w9_transition_end_iter ${W9_TRANSITION_END} \
--risk_calibration_w8_enabled 0 \
--risk_calibration_w7_enabled 0 \
--risk_calibration_w5_soft_only_enabled 0 \
--risk_calibration_w6_late_tail_enabled 0"

echo "Starting ODOC W9 run (W4-style bootstrap before ${W9_TRANSITION_START}, single-path control anneal to ${W9_TRANSITION_END}, then pure W1')"
...
echo "W9_TRANSITION_START=${W9_TRANSITION_START} W9_TRANSITION_END=${W9_TRANSITION_END}"
...
wait
echo "ODOC W9 run finished. EXP_NAME=${EXP_NAME}"
