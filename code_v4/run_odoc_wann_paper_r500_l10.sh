#!/bin/bash
# Official FedLPPA + WANN on ODOC.

#SBATCH --job-name=FedLPPA_ODOC_WANN
...
RUN_PREFIX="${RUN_PREFIX:-official_wann_odoc_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8511}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting official FedLPPA + WANN ODOC run"
...
wait
echo "Official FedLPPA + WANN ODOC run finished. EXP_NAME=${EXP_NAME}"
