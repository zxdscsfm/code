#!/bin/bash
# Official FedLPPA + WANN + RGFTD-v3.5 stable-teacher routing on POLYP.

#SBATCH --job-name=Sparse5_POLYP
...
RUN_PREFIX="${RUN_PREFIX:-wann_rgftd_polyp_sparse_scribble5_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8534}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
ROOT_PATH="${ROOT_PATH:-/data/jianbingshen/yanghongji/FedLPPA_Original/data/POLYP_h5}"
...
echo "Starting official FedLPPA + WANN + RGFTD-v3.5 stable POLYP run"
...
wait
echo "Official FedLPPA + WANN + RGFTD-v3.5 stable POLYP run finished. EXP_NAME=${EXP_NAME}"
