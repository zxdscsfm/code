#!/bin/bash
# POLYP V1B + adaptive geometry:
# keep the full v1 backbone-anchored governance path with uniform base geometry,
# and enable client-level adaptive geometry interpolation directly in BASE_ARGS.

#SBATCH --job-name=FedLPPA_POLYP_V1B-adgeo
...
METHOD_TAG="${METHOD_TAG:-v1b-adgeo_polyp_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8210}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--adaptive_geometry_enabled 1 \
--adaptive_geometry_prior_mode linear \
--adaptive_geometry_prior_alpha 0.8 \
--adaptive_geometry_prior_floor 0.2 \
--adaptive_geometry_init_lambda 0.5 \
--adaptive_geometry_warmup_iters 200 \
--adaptive_geometry_min_pixels 64 \
--adaptive_geometry_support_weight 0.4 \
--adaptive_geometry_expansion_weight 0.4 \
--adaptive_geometry_agreement_weight 0.2 \
--adaptive_geometry_slow_up_momentum 0.95 \
--adaptive_geometry_fast_down_momentum 0.70 \
...
echo "Starting POLYP V1B adaptive-geometry run"
echo "EXP_NAME=${EXP_NAME}"
echo "LOG_DIR=${LOG_DIR}"
echo "METHOD_TAG=${METHOD_TAG}"
echo "SEED=${SEED}"
echo "SERVER_ADDRESS=${SERVER_ADDRESS}"
echo "ADAPTIVE_TAU_UPDATE=${ADAPTIVE_TAU_UPDATE}"
...
wait
echo "POLYP V1B adaptive-geometry run finished. EXP_NAME=${EXP_NAME}"
