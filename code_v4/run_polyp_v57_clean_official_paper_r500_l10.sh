#!/bin/bash
# External launcher for a clean official FedLPPA baseline on POLYP.

#SBATCH --job-name=FedLPPA_Clean_POLYP
...
RUN_PREFIX="${RUN_PREFIX:-official_clean_polyp_paper_r500_l10}"
RUN_TAG="${RUN_PREFIX}_$(date +%Y%m%d_%H%M%S)"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8164}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting clean official FedLPPA POLYP baseline"
...
wait
echo "Clean official FedLPPA POLYP baseline finished. EXP_NAME=${EXP_NAME}"
