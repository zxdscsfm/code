#!/bin/bash
# POLYP W7:
# transplant the W7 bootstrap-to-W1' hard-cut schedule onto POLYP under paper-aligned r500/l10.

#SBATCH --job-name=FedLPPA_POLYP_W7
...
METHOD_TAG="${METHOD_TAG:-v58_polyp_w7_w42bootstrap_w1p_hardcut_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8184}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
EVAL_ITERS="${EVAL_ITERS:-10}"
TSNE_ITERS="${TSNE_ITERS:-200}"
MAX_ITERATIONS="${MAX_ITERATIONS:-5000}"
ROOT_PATH="${ROOT_PATH:-/data/jianbingshen/yanghongji/FedLPPA_github_official_full/data/POLYP_h5}"
ADAPTIVE_TAU_UPDATE="${ADAPTIVE_TAU_UPDATE:-1}"
ADAPTIVE_RESUME_STATE="${ADAPTIVE_RESUME_STATE:-0}"

RUN_TAG="${METHOD_TAG}_polyp_$(date +%Y%m%d_%H%M%S)_seed${SEED}"
...
--risk_calibration_w7_enabled 1 \
--risk_calibration_w7_switch_iter 800 \
--risk_calibration_w5_soft_only_enabled 0 \
--risk_calibration_w6_late_tail_enabled 0"

echo "Starting POLYP W7 run"
...
wait
echo "POLYP W7 run finished. EXP_NAME=${EXP_NAME}"
