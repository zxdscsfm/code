#!/bin/bash
# PROSTATE cleanV1:
# unified clean backbone route:
# L = g_hat * L_v1 + (1 - g_hat) * L_v1B
# where v1 is restored to the original fixed geometry profile.

#SBATCH --job-name=FedLPPA_PROSTATE_cleanV1
...
METHOD_TAG="${METHOD_TAG:-cleanV1_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8402}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--unified_backbone_enabled 1 \
--unified_backbone_gate_power 2.0 \
--unified_backbone_v1_geometry_pseudo_weights 1.0,0.8,0.5,0.2 \
--unified_backbone_v1_tau_update 1 \
--unified_backbone_v1b_geometry_pseudo_weights 1.0,1.0,1.0,1.0 \
--unified_backbone_v1b_tau_update 0 \
${EXTRA_ARGS}"

echo "Starting PROSTATE cleanV1 run"
...
echo "PROSTATE cleanV1 run finished. EXP_NAME=${EXP_NAME}"
