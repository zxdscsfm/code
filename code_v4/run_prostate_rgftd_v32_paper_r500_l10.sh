#!/bin/bash
# Official FedLPPA + WANN + RGFTD-v3.2 on PROSTATE.
# V3.2 keeps v1-fix2/v2.2 pixel safety and adds benefit-aware teacher leases.
#SBATCH --job-name=FedLPPA_PROSTATE_RGFTD_V32
...
export RUN_PREFIX="${RUN_PREFIX:-official_rgftd_v32_prostate_paper_r500_l10}"
...
if [ -n "${EXTRA_ARGS:-}" ]; then
    export EXTRA_ARGS="${V22_ARGS} ${V3_ARGS} ${V32_ARGS} ${EXTRA_ARGS}"
else
    export EXTRA_ARGS="${V22_ARGS} ${V3_ARGS} ${V32_ARGS}"
fi
