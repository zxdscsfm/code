#!/bin/bash
# PROSTATE SAR2:
# dg2 geometry routing into base supervision + FedSAR semantic prototype branch
# + cold-start-safe prototype producer gating.

#SBATCH --job-name=FedLPPA_PROSTATE_SAR2
...
METHOD_TAG="${METHOD_TAG:-sar2_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8393}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--geometry_pseudo_weights 1.0,1.0,1.0,1.0 \
--trustgeo_enabled 0 \
--dg_enabled 1 \
--dg_max_audit_samples 64 \
--dg_max_prop_samples 16 \
--dg_local_std_kernel 15 \
--dg_far_radius 24.0 \
--dg_sep_near_radius 8.0 \
--dg_sep_mid_radius 24.0 \
--dg_sep_far_radius 48.0 \
--dg_min_fg_pixels 8 \
--dg_min_ring_pixels 64 \
...
--fedsar_enabled 1 \
--fedsar_geometry_gate_power 2.0 \
--fedsar_proto_warmup_rounds 800 \
--fedsar_proto_ramp_rounds 800 \
--fedsar_proto_max_preview_batches 8 \
--fedsar_proto_count_tau 64.0 \
--fedsar_proto_var_tau 0.25 \
--fedsar_proto_align_min 0.20 \
--fedsar_proto_temp 0.10 \
--fedsar_proto_margin_thr 0.15 \
--fedsar_anchor_min_pixels 2 \
--fedsar_proto_anchor_gamma 0.20 \
--fedsar_server_proto_min_count 4 \
--fedsar_server_proto_var_tau 0.25 \
--fedsar_server_proto_align_min 0.10 \
--fedsar_server_proto_compact_min 0.30 \
--fedsar_server_proto_align_gate_min 0.20 \
--fedsar_server_proto_bootstrap_align_gate_min 0.20 \
--fedsar_server_proto_bootstrap_min_supporters 2 \
--fedsar_server_proto_align_floor 0.25 \
--fedsar_server_proto_bootstrap_rounds 800 \
${EXTRA_ARGS}"

echo "Starting PROSTATE SAR2 run"
...
echo "PROSTATE SAR2 run finished. EXP_NAME=${EXP_NAME}"
