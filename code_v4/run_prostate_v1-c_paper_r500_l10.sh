#!/bin/bash
# PROSTATE V1-C:
# static client-level geometry qualification that interpolates between
# the stable v1B governance path and a fixed linear geometry prior.

#SBATCH --job-name=FedLPPA_PROSTATE_V1-C
...
METHOD_TAG="${METHOD_TAG:-v1-c_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8258}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting PROSTATE V1-C run"
...
wait
echo "PROSTATE V1-C run finished. EXP_NAME=${EXP_NAME}"
