#!/bin/bash
# PROSTATE V1-supportbonus:
# keep the stable v1B governance path, and add only a conservative
# local support bonus derived from static weak-label structure quality.

#SBATCH --job-name=FedLPPA_PROSTATE_V1-SBONUS
...
METHOD_TAG="${METHOD_TAG:-v1-supportbonus_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8228}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--support_bonus_enabled 1 \
--support_bonus_tau_c 0.05 \
--support_bonus_tau_d 0.20 \
--support_bonus_dilation_radius 5 \
--support_bonus_density_kernel 11 \
--support_bonus_support_radius 5 \
--support_bonus_lambda 0.10 \
...
echo "Starting PROSTATE V1-supportbonus run"
...
wait
echo "PROSTATE V1-supportbonus run finished. EXP_NAME=${EXP_NAME}"
