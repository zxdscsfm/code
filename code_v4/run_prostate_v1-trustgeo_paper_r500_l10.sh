#!/bin/bash
# PROSTATE V1-trustgeo:
# keep the stable v1B governance path, but replace the fixed uniform
# geometry ablation with a client-level static trust score that scales
# a low-degree linear geometry prior.

#SBATCH --job-name=FedLPPA_PROSTATE_V1-TRUSTGEO
...
METHOD_TAG="${METHOD_TAG:-v1-trustgeo_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8218}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting PROSTATE V1-trustgeo run"
...
wait
echo "PROSTATE V1-trustgeo run finished. EXP_NAME=${EXP_NAME}"
