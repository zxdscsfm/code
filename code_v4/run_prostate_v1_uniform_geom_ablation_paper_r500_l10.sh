#!/bin/bash
# PROSTATE V1 uniform-geometry ablation:
...
#SBATCH --job-name=FedLPPA_PROSTATE_V1B
...
METHOD_TAG="${METHOD_TAG:-v1b_prostate_uniform_geom_ablation_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8198}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
echo "Starting PROSTATE V1 uniform-geometry ablation run"
...
wait
echo "PROSTATE V1 uniform-geometry ablation run finished. EXP_NAME=${EXP_NAME}"
