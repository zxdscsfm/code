#!/bin/bash
# PROSTATE V1B + adaptive geometry, Experiment A:
# force prostate keypoint clients onto the pure v1B uniform-geometry path
# while keeping adaptive geometry active for the other prostate clients.

#SBATCH --job-name=FedLPPA_PROSTATE_V1B-adgeo-kp0
...
METHOD_TAG="${METHOD_TAG:-v1b-adgeo_kp0_prostate_paper_r500_l10}"
...
--adaptive_geometry_force_uniform_prostate_keypoint 1 \
...
echo "Starting PROSTATE V1B adaptive-geometry Experiment A (keypoint forced uniform)"
...
wait
echo "PROSTATE V1B adaptive-geometry Experiment A finished. EXP_NAME=${EXP_NAME}"
