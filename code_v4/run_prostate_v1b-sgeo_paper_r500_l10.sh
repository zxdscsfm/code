#!/bin/bash
# PROSTATE V1B + scalar learnable geometry strength:
# keep the original v1B uniform-geometry path as the safe backbone,
# then learn a single local scalar s_client that interpolates between
# uniform weighting and a fixed linear geometry prior template.

#SBATCH --job-name=FedLPPA_PROSTATE_V1B-sgeo
...
METHOD_TAG="${METHOD_TAG:-v1b-sgeo_prostate_paper_r500_l10}"
SERVER_ADDRESS="${SERVER_ADDRESS:-127.0.0.1:8214}"
SEED="${SEED:-2022}"
ITERS="${ITERS:-10}"
...
--learnable_geometry_strength_enabled 1 \
--learnable_geometry_strength_init 0.0 \
--learnable_geometry_strength_warmup_iters 800 \
--learnable_geometry_strength_lr_scale 0.05 \
--learnable_geometry_strength_reg_lambda 1e-3 \
--learnable_geometry_prior_alpha 0.8 \
--learnable_geometry_prior_floor 0.2 \
--adaptive_geometry_enabled 0 \
...
echo "Starting PROSTATE V1B + scalar learnable geometry strength run"
...
wait
echo "PROSTATE V1B + scalar learnable geometry strength run finished. EXP_NAME=${EXP_NAME}"
