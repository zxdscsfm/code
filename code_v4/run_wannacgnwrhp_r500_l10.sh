#!/bin/bash
# WANN + ACG-NWR-HP under the same r500/l10 weak-label protocols.
...
DATASET="${DATASET:-}"
if [ -z "${DATASET}" ]; then
    echo "DATASET must be one of: prostate, polyp, faz, isic"
    exit 1
fi
...
RUN_PREFIX="${RUN_PREFIX:-wannacgnwrhp_${DATASET}_r500_l10}"
case "${DATASET}" in
    prostate)
...
echo "Starting WANN + ACG-NWR-HP ${DATASET} run"
...
echo "WANN + ACG-NWR-HP ${DATASET} run finished. EXP_NAME=${EXP_NAME}"
