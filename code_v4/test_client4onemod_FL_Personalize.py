...
parser.add_argument('--gpu', type=str, default='3',
                    help='CUDA visible devices for evaluation')
...
parser.add_argument('--dual_init', type=str, default='aggregated',
...
def get_client_ids_polyp(client,base_dir):
    ...
    if client == "client0":
        ...
        return "ERROR KEY"
def get_client_ids_prostate(client,base_dir):
    client1_test_set = 'Domain1/test/'+pd.Series(os.listdir(base_dir+"/Domain1/test"))
    client1_training_set = 'Domain1/train/'+pd.Series(os.listdir(base_dir+"/Domain1/train"))
    client2_test_set = 'Domain2/test/'+pd.Series(os.listdir(base_dir+"/Domain2/test"))
    client2_training_set = 'Domain2/train/'+pd.Series(os.listdir(base_dir+"/Domain2/train"))
    ...
    client5_test_set = 'Domain5/test/'+pd.Series(os.listdir(base_dir+"/Domain5/test"))
    client5_training_set = 'Domain5/train/'+pd.Series(os.listdir(base_dir+"/Domain5/train"))
    client6_test_set = 'Domain6/test/'+pd.Series(os.listdir(base_dir+"/Domain6/test"))
    client6_training_set = 'Domain6/train/'+pd.Series(os.listdir(base_dir+"/Domain6/train"))
    client1_test_set = client1_test_set.tolist()
    client1_training_set = client1_training_set.tolist()
    client2_test_set = client2_test_set.tolist()
    client2_training_set = client2_training_set.tolist()
    ...
    if client == "client0":
        ...
        return [client6_training_set, client6_test_set]
    elif client == 'client_all':
        client_train_total=[]
        client_test_total=[]
        for i in client1_training_set:
...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0:
        if gt.sum() > 0:
            ...
            return 0, 0, 192, 0, 0, 0, 0, 0 # 384*384
            # return 0, 0, 128, 0, 0, 0, 0, 0 # 256*256
    else:
...
def Inference(FLAGS):
    ...
    if snapshot_path is None or snapshot_path == parser.get_default('snapshot_path'):
    ...
    client_index = None
    client_match = re.match(r'^client(\d+)$', str(FLAGS.client))
    if client_match is not None:
        client_index = int(client_match.group(1))

    candidate_paths = []
    if client_index is not None:
        candidate_paths.append(
            os.path.join(snapshot_path, 'client_{}_{}_best_model.pth'.format(client_index, FLAGS.model))
        )
    candidate_paths.append(
        os.path.join(snapshot_path, '{}_{}_best_model.pth'.format(FLAGS.client, FLAGS.model).replace("client", "client_"))
    )
    candidate_paths.append(
        os.path.join(snapshot_path, '{}_async_{}_best_model.pth'.format(FLAGS.client, FLAGS.model).replace("client", "client_"))
    )
    candidate_paths.append(os.path.join(snapshot_path, 'unet_best_model.pth'))
    candidate_paths.append(os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model)))

    save_mode_path = None
    for candidate_path in candidate_paths:
        if os.path.exists(candidate_path):
            save_mode_path = candidate_path
            break
    if save_mode_path is None:
        raise FileNotFoundError(
            "No compatible checkpoint found in {}. Tried: {}".format(snapshot_path, candidate_paths)
        )
