...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0:
        if gt.sum() > 0:
            ...
            return 0, 192, 0, 0, 0, 0, 1 # 384*384
            # return 0, 128, 0, 0, 0, 0, 1 # 256*256
    else:
...
def test_single_volume_nomain(image, label, net, classes, patch_size=[256, 256], amp=False):
    image, label = image.squeeze(1).squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(1).squeeze(0).cpu().detach().numpy()
    # ###odoc val
    if len(image.shape) == 3:
        ...
        with torch.no_grad():
            with autocast(enabled=amp):
                out = torch.argmax(torch.softmax(
                    net(input)[8], dim=1), dim=1).squeeze(1).squeeze(0)
                out = out.cpu().detach().numpy()
                    # pred = zoom(
                    #     out, (x / patch_size[0], y / patch_size[1]), order=0)
                prediction = out
