...
def _build_debug_artifacts(student_logits, teacher_logits, label, wann_maps, args, iter_num, image):
    ...
    return {
        ...
        "student_conf": student_conf.detach(),
...
def _render_requested_case_panel(image, label, case_name, teacher_name, target_name, mode, artifacts, out_path):
    img = _norm_image(image)
    weak_panel = _overlay_mask(
        img,
        [
    ...
    axes[0].imshow(weak_panel)
    axes[0].set_title("Weak annotation")

    im1 = axes[1].imshow(artifacts["student_conf"], cmap="viridis", vmin=0.0, vmax=1.0)
    axes[1].set_title("Prediction confidence")
    plt.colorbar(im1, ax=axes[1], fraction=0.046, pad=0.04)

    im2 = axes[2].imshow(artifacts["reliability"], cmap="viridis", vmin=0.0, vmax=1.0)
    axes[2].set_title("WANN reliability map")
    plt.colorbar(im2, ax=axes[2], fraction=0.046, pad=0.04)

    axes[3].imshow(partition_panel)
    axes[3].set_title("Core / Soft / Ignore")

    im4 = axes[4].imshow(artifacts["teacher_fg_prob"], cmap="magma", vmin=0.0, vmax=1.0)
    axes[4].set_title("Teacher refinement before")
    plt.colorbar(im4, ax=axes[4], fraction=0.046, pad=0.04)

    im5 = axes[5].imshow(artifacts["refined_q_fg"], cmap="magma", vmin=0.0, vmax=1.0)
    axes[5].set_title("Teacher refinement after")
    plt.colorbar(im5, ax=axes[5], fraction=0.046, pad=0.04)

    axes[6].imshow(final_panel)
    axes[6].set_title("Final segmentation")

    axes[7].imshow(raw_panel)
    axes[7].set_title("Released region")
    ...
    fig.suptitle(
        f"{teacher_name}->{target_name} | {mode} | case={case_name}",
        fontsize=11,
        y=0.98,
    )
...
def _roi_bbox_from_masks(*masks, margin=36):
    ys = []
    xs = []
    for mask in masks:
        if mask is None:
    ...
    y0 = max(int(y_all.min()) - margin, 0)
    y1 = int(y_all.max()) + margin + 1
    x0 = max(int(x_all.min()) - margin, 0)
    x1 = int(x_all.max()) + margin + 1
    return y0, y1, x0, x1


def _render_focus_case_panel(image, case_name, teacher_name, target_name, mode, artifacts, out_path):
    img = _norm_image(image)
    raw_mask = (artifacts["active_fg_mask"] > 0.5).astype(np.float32)
    refined_prob = artifacts["refined_q_fg"]
    teacher_prob = artifacts["teacher_fg_prob"]
    final_mask = (artifacts["student_pred"] > 0.5).astype(np.float32)
    weak_seed = (artifacts["seed_fg_support"] > 0.5).astype(np.float32)

    refined_mask = refined_prob >= 0.35
    teacher_mask = teacher_prob >= 0.35
    bbox = _roi_bbox_from_masks(raw_mask, refined_mask, final_mask, weak_seed, margin=44)
    if bbox is None:
        bbox = (0, img.shape[0], 0, img.shape[1])
    y0, y1, x0, x1 = bbox

    delta = refined_prob - teacher_prob
    overlay_img = np.stack([img, img, img], axis=-1)

    fig, axes = plt.subplots(1, 3, figsize=(15.5, 5.2), dpi=240)

    axes[0].imshow(overlay_img)
    axes[0].contour(raw_mask, levels=[0.5], colors=["#08d9d6"], linewidths=2.2)
    axes[0].contour(refined_mask.astype(np.float32), levels=[0.5], colors=["#ffd166"], linewidths=2.2)
    axes[0].contour(final_mask, levels=[0.5], colors=["#ef476f"], linewidths=2.4)
    axes[0].imshow(
        np.ma.masked_where(~weak_seed.astype(bool), weak_seed),
        cmap="Reds",
        alpha=0.75,
        vmin=0.0,
        vmax=1.0,
    )
    rect = plt.Rectangle((x0, y0), x1 - x0, y1 - y0, fill=False, edgecolor="white", linewidth=1.5, linestyle="--")
    axes[0].add_patch(rect)
    axes[0].set_title("Full view: raw vs refined vs final", fontsize=11)

    axes[1].imshow(overlay_img[y0:y1, x0:x1])
    axes[1].contour(raw_mask[y0:y1, x0:x1], levels=[0.5], colors=["#08d9d6"], linewidths=2.6)
    axes[1].contour(refined_mask[y0:y1, x0:x1].astype(np.float32), levels=[0.5], colors=["#ffd166"], linewidths=2.6)
    axes[1].contour(final_mask[y0:y1, x0:x1], levels=[0.5], colors=["#ef476f"], linewidths=2.8)
    axes[1].imshow(
        np.ma.masked_where(~weak_seed[y0:y1, x0:x1].astype(bool), weak_seed[y0:y1, x0:x1]),
        cmap="Reds",
        alpha=0.75,
        vmin=0.0,
        vmax=1.0,
    )
    axes[1].set_title("Zoomed ROI", fontsize=11)

    im2 = axes[2].imshow(delta[y0:y1, x0:x1], cmap="coolwarm", vmin=-0.6, vmax=0.6)
    axes[2].contour(weak_seed[y0:y1, x0:x1], levels=[0.5], colors=["#111111"], linewidths=1.2)
    axes[2].set_title("Refinement delta (after - before)", fontsize=11)
    plt.colorbar(im2, ax=axes[2], fraction=0.046, pad=0.04)
    ...
    legend_lines = [
        plt.Line2D([0], [0], color="#08d9d6", lw=2.4, label="raw release"),
        plt.Line2D([0], [0], color="#ffd166", lw=2.4, label="refined q"),
        plt.Line2D([0], [0], color="#ef476f", lw=2.4, label="final pred"),
    ]
    axes[1].legend(handles=legend_lines, loc="lower right", frameon=True, fontsize=9)
...
def _render_simple4_case_panel(image, case_name, teacher_name, target_name, mode, artifacts, out_path):
    img = _norm_image(image)
    weak_seed = (artifacts["seed_fg_support"] > 0.5).astype(bool)
    raw_mask = (artifacts["active_fg_mask"] > 0.5).astype(bool)
    refined_mask = (artifacts["refined_q_fg"] >= 0.35).astype(bool)
    final_mask = (artifacts["student_pred"] > 0.5).astype(bool)

    bbox = _roi_bbox_from_masks(weak_seed, raw_mask, refined_mask, final_mask, margin=52)
    if bbox is None:
        bbox = (0, img.shape[0], 0, img.shape[1])
    y0, y1, x0, x1 = bbox
    ...
    weak_panel = _overlay_mask(
        img_roi,
        [((weak_seed[y0:y1, x0:x1]), (0.95, 0.12, 0.12))],
        alpha=0.78,
    )
    raw_panel = _overlay_mask(
        img_roi,
        [((raw_mask[y0:y1, x0:x1]), (0.05, 0.82, 0.86))],
        alpha=0.65,
    )
    refined_panel = _overlay_mask(
        img_roi,
        [((refined_mask[y0:y1, x0:x1]), (1.0, 0.82, 0.10))],
        alpha=0.68,
    )
    final_panel = _overlay_mask(
        img_roi,
        [((final_mask[y0:y1, x0:x1]), (0.92, 0.20, 0.34))],
        alpha=0.68,
    )

    panels = [
        ("Weak annotation", weak_panel),
        ("Raw release", raw_panel),
        ("Refined q", refined_panel),
        ("Final segmentation", final_panel),
    ]

    fig, axes = plt.subplots(1, 4, figsize=(14.4, 3.8), dpi=260)
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.94], pad=0.4)
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    with torch.no_grad():
        for teacher_name, target_name, mode in parsed_pairs:
                ...
                if best is None or score > best["score"]:
                    best = {
                        ...
                        "artifacts": {
                            ...
                            "student_conf": _to_cpu_numpy(artifacts["student_conf"][0]),
            ...
            requested_out_path = os.path.join(args.output_dir, f"requested_{out_name}")
            focus_out_path = os.path.join(args.output_dir, f"focus_{out_name}")
            simple4_out_path = os.path.join(args.output_dir, f"simple4_{out_name}")
            _render_case_panel(
                image=best["image"],
                label=best["label"],
            ...
            row = {
                ...
                "requested_figure": requested_out_path,
                "focus_figure": focus_out_path,
                "simple4_figure": simple4_out_path,
            }
