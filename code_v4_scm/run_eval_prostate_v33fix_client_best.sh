#!/bin/bash
# Evaluate the current PROSTATE v3.3-fix run with per-client best checkpoints.

#SBATCH --job-name=EVAL_PROSTATE_V33FIX
...
SRC_EXP="${SRC_EXP:-prostate/FedLPPA_official_rgftd_v33_fix_prostate_paper_r500_l10_20260511_063159_seed2022}"
EVAL_TAG="${EVAL_TAG:-prostate/eval_v33_fix_client_best_$(date +%Y%m%d_%H%M%S)}"
MODEL_ROOT="../model"
SRC_DIR="${MODEL_ROOT}/${SRC_EXP}"
LOG_DIR="logs/run_eval_v33_fix_prostate_client_best_$(date +%Y%m%d_%H%M%S)"
mkdir -p "${LOG_DIR}"
...
with open(selected_path, "w", encoding="utf-8") as out:
    for cid in range(6):
        ...
        for path in glob.glob(os.path.join(src_dir, f"client_{cid}*_iter_*_dice_*.pth")):
            ...
            # Prefer the checkpoint with the best recorded client dice; async wins ties.
...
SUP_TYPES=(block keypoint scribble keypoint scribble box)
