#!/bin/bash
# WANN + ACG-SDG under the same r500/l10 weak-label protocols.
...
RUN_PREFIX="${RUN_PREFIX:-wannacgsdg_${DATASET}_r500_l10}"
case "${DATASET}" in
    prostate)
...
echo "Starting WANN + ACG-SDG ${DATASET} run"
...
echo "WANN + ACG-SDG ${DATASET} run finished. EXP_NAME=${EXP_NAME}"
